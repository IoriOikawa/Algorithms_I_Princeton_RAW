...
/**
 * This is a 2d-tree implementation for princeton's Algorithms I.
 * A mutable data type that uses a 2d-tree to implement
 * the same API (but replace {@code PointSET} with {@code KdTree}). 
 * The idea is to build a BST with points in the nodes,
 * using the coordinates of the points as
 * keys in strictly alternating sequence.
 *
 * refined by oikawa iori
 */
...
public class KdTree {
    ...
    private static class Node {
        ...
        // Turning the coordinace into the shape I can recognize...
    ...
    public void insert(Point2D p) {
        checkNull(p);
        // (If the point has not been in the set already...
        if (root == null) {
     ...
     * Implemented by the question already? Don't know...
     */
    public void draw() {

    }

     // Fetching all points that are inside the rectangle...
    ...
    public Iterable<Point2D> range(RectHV rect) {
     ...
     // Adding all points under target node using DFS...
    ...
    private void addAll(Node node, RectHV rect, List<Point2D> results) {
     ...
     // For those nearest neighbors in the set to point p; if empty, return null.
